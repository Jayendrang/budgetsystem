...
public class ExpensesDaoImpl implements ExpenesesDao {
		...
		try {
			for (Expenses expense : expenses) {
				...
				status = template.update(insertQuery,
						new Object[] { ApplicationUtilities.getRandomExpensesID(), session.getUserName(),
								expense.getExpenses_date(), expense.getExpenses_name(), expense.getExpenses_type(),
								expense.getExpenses_desc(), Double.parseDouble(expense.getAmount()),
								expense.getRemark(), ApplicationUtilities.getCurrentDate() });
	...
	public boolean updateExpenses(Expenses expenses, SessionInfo session) {
		...
		try {
			...
			int status = template.update(udpateQueryExpenses,
					new Object[] { expenses.getExpenses_date(), expenses.getExpenses_name(),
							expenses.getExpenses_type(), expenses.getExpenses_desc(),
							Double.parseDouble(expenses.getAmount()), expenses.getRemark(), expenses.getExpenses_id(),
							session.getUserName() });
			result = status > 0 ? true : false;
