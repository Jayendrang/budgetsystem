...
public class EventReminderDaoImpl implements SpringSemester.budgetsystem.dao.UserEventReminderDao {
	...
	public boolean insertNewUserEvent(List<UserEvent> reminder, SessionInfo session) {
		...
		rowcount =+ jdbcTemplate.update(insertQuery,new Object[]{
				ApplicationUtilities.getReminderID(),
				session.getUserName(),
				userEvent.getEvent_desc(),
				userEvent.getEvent_start_date(),
	...
	public boolean updateReminderEvent(UserEvent reminder, SessionInfo session) {
		...
		rowcount=jdbcTemplate.update(updateQuery,new Object[] {
				reminder.getEvent_desc(),
				reminder.getEvent_start_date(),
				reminder.getCreated_on(),
				session.getUserName(),
				reminder.getEventid()
				});
		status=rowcount>0?true:false;
	...
	public List<UserEvent> getAllUserEvents( SessionInfo session) {
		...
		String selectQuery="select event_id,user_id,event_desc,event_start_date,created_on from scheduled_event_details where user_id=?";	
			userReminders = jdbcTemplate.queryForObject(selectQuery, new Object[] {session.getUserName()},new ReminderDataMapper());
		}catch(Exception ex) {
			ex.printStackTrace();
		}
	...
	class ReminderDataMapper implements RowMapper<List<UserEvent>>{
		...
		public List<UserEvent> mapRow(ResultSet result, int arg1) throws SQLException{
			...
			reminder.setEventid(result.getString(""));
			reminder.setUserid(result.getString(""));
			reminder.setEvent_start_date(result.getString(""));
			reminder.setEvent_desc(result.getString(""));
